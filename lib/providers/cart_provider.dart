...
class CartProvider extends ChangeNotifier {
  ...
  // ============ 独立物流单库（v1.9.149） ============
  // 抓包 JSON 导入时，所有带运单号的物流记录不论订单在不在本地都进这里，
  // 「物流」页（快递库）单独列出；与订单列表完全解耦。

  final List<ExpressRecord> _expressRecords = [];

  /// 全部独立物流单（最新轨迹时间靠前的在前，无轨迹按写入时间）
  List<ExpressRecord> get expressRecords {
    final sorted = [..._expressRecords]..sort((a, b) {
        final ta = a.latestTraceTime;
        final tb = b.latestTraceTime;
  ...
  int get expressRecordsCount => _expressRecords.length;

  /// 该订单号是否已存在于本地订单列表（物流单"已关联订单"角标用）
  bool orderNoExists(String orderNo) {
  ...
  void _persistExpress() {
    PersistenceService.saveExpressRecords(_expressRecords);
  }

  /// 批量并入物流单（按运单号去重）：
  /// - 新单号 → 直接入库；
  /// - 已有单号 → 新抓包轨迹非空且不同则整条刷新（抓包=最新事实），
  ///   否则只补空字段（公司/logo/电话/摘要等），绝不抹掉已有数据。
  /// 返回实际入库/更新条数。
  int upsertExpressRecords(List<ExpressRecord> incoming) {
    var changed = 0;
    for (final rec in incoming) {
      final no = rec.waybillNo.trim();
      if (no.isEmpty) continue;
      final i = _expressRecords.indexWhere((e) => e.waybillNo == no);
      if (i < 0) {
        _expressRecords.add(rec);
        changed++;
        continue;
      }
      final old = _expressRecords[i];
      if (rec.tracesJson.isNotEmpty && rec.tracesJson != old.tracesJson) {
        // 新抓包整体刷新，但旧字段里抓包没有的（如手动补的客服电话）保留
        _expressRecords[i] = rec.copyWith(
          shipCompany: rec.shipCompany.isNotEmpty ? null : old.shipCompany,
          shipLogo: rec.shipLogo.isNotEmpty ? null : old.shipLogo,
          shipPhone: rec.shipPhone.isNotEmpty ? null : old.shipPhone,
        );
        changed++;
        continue;
      }
      // 只补空
      var touched = false;
      if (old.shipCompany.isEmpty && rec.shipCompany.isNotEmpty) {
        old.shipCompany = rec.shipCompany;
        touched = true;
      }
      if (old.shipLogo.isEmpty && rec.shipLogo.isNotEmpty) {
        old.shipLogo = rec.shipLogo;
        touched = true;
      }
      if (old.shipPhone.isEmpty && rec.shipPhone.isNotEmpty) {
        old.shipPhone = rec.shipPhone;
        touched = true;
      }
      if (old.logistics.isEmpty && rec.logistics.isNotEmpty) {
        old.logistics = rec.logistics;
        touched = true;
      }
      if (old.tracesJson.isEmpty && rec.tracesJson.isNotEmpty) {
        old.tracesJson = rec.tracesJson;
        touched = true;
      }
      if (old.orderNo.isEmpty && rec.orderNo.isNotEmpty) {
        old.orderNo = rec.orderNo;
        touched = true;
      }
      if (old.shopName.isEmpty && rec.shopName.isNotEmpty) {
        old.shopName = rec.shopName;
        touched = true;
      }
      if (old.title.isEmpty && rec.title.isNotEmpty) {
        old.title = rec.title;
        touched = true;
      }
      if (old.imageUrl.isEmpty && rec.imageUrl.isNotEmpty) {
        old.imageUrl = rec.imageUrl;
        touched = true;
      }
      if (touched) {
        old.updatedAt = DateTime.now().toIso8601String();
        changed++;
      }
    }
    if (changed > 0) {
  ...
  /// 更新单条物流单（物流详情页联网刷新/改单号后回写），按运单号替换
  void updateExpressRecord(ExpressRecord rec) {
    final i = _expressRecords.indexWhere((e) => e.waybillNo == rec.waybillNo);
    if (i < 0) {
      _expressRecords.add(rec);
    } else {
      _expressRecords[i] = rec;
    }
    _persistExpress();
    notifyListeners();
  }
  ...
  /// 删除单条物流单
  void deleteExpressRecord(String waybillNo) {
    final n = _expressRecords.length;
    _expressRecords.removeWhere((e) => e.waybillNo == waybillNo);
    if (_expressRecords.length != n) {
      _persistExpress();
      notifyListeners();
    }
  }
  ...
  /// 清空物流单库
  Future<void> clearExpressRecords() async {
    if (_expressRecords.isEmpty) return;
    _expressRecords.clear();
    await PersistenceService.saveExpressRecords(const []);
    notifyListeners();
  }
  ...
  /// v1.9.149：addOrders=false（纯物流 JSON）时只给已有订单补物流 +
  ...
  ({int added, int skipped, int blocked, int logiFilled, int expressUpserted})
    ...
    // v1.9.149：抓包里所有带运单号的物流单，不论订单存不存在先进独立物流库
    // （放在最前面：即使订单全部被跳过/拦截，物流单也已入库）
    final expressUpserted = upsertExpressRecords([
      for (final shop in incoming)
        for (final it in shop.items)
          if (it.waybillNo.trim().isNotEmpty)
            ExpressRecord.fromOrderItem(it, shopName: shop.shopName),
    ]);
    for (final shop in incoming) {
      ...
      for (final it in shop.items) {
        ...
        for (final old in _shops) {
          for (final oldItem in old.items) {
            ...
            // v1.9.147：订单级头像回填——真实淘宝按订单存店铺头像快照。
            // 默认只补空（手动/预置的订单级头像不覆盖）；
            // forceAvatar 打开时以本次抓包为准无条件刷
            if (shop.shopAvatar.isNotEmpty &&
                (forceAvatar || oldItem.shopAvatar.isEmpty)) {
              oldItem.shopAvatar = shop.shopAvatar;
              opsBackfilled = true;
            }
      ...
      // v1.9.149：纯物流导入到此为止——物流单已在函数开头进独立物流库，
      // 已有订单的物流也在上面回填完，不再新增订单卡片
      if (!addOrders) continue;
      ...
      for (final it in newItems) {
        ...
        // v1.9.147：新增订单盖上本次抓包的订单级头像（抓包 JSON 只带
        // 店铺级头像，解析时没有订单维度——此单抓包时店铺长什么样，
        // 这个订单就固定用什么，之后店铺改头像不影响它）
        if (it.shopAvatar.isEmpty && shop.shopAvatar.isNotEmpty) {
          it.shopAvatar = shop.shopAvatar;
        }
  ...
  Future<void> _restoreFromDisk() async {
    try {
      ...
      // v1.9.149：加载独立物流单库（与订单解耦的抓包物流单）
      _expressRecords
        ..clear()
        ..addAll(await PersistenceService.loadExpressRecords());
  ...
  ShoppingCartShop cloneShop(ShoppingCartShop shop) {
    ...
    // 深拷贝原样带走源单的订单级/店铺级头像即可，不再用"同店最新
    // 抓包头像"覆盖（会导致克隆订单头像和源订单对不上，用户反馈）
